...
Menggunakan: Haar Cascade (Viola-Jones) + DNN  [GPU-Accelerated]
================================================================================
Kontrol:
...
# ==============================
MODEL_CANDIDATES = [
    "models_regularized_imgaug/hog_dnn_imgaug_savedmodel",
    "models_regularized_imgaug/hog_dnn_imgaug.keras",
    "models_regularized_imgaug/hog_dnn_imgaug.h5",
]
...
LOG_DIR            = "log_haarcascade_dnn_csv"
LOG_FILE_BASE_NAME = "log_haar_dnn"
...
DETECTOR_NAME = "HaarCascade"
FEATURE_NAME  = "Haar Cascade (Viola-Jones)"
...
REPORT_FILE  = os.path.join(LOG_DIR, f"report_haar_dnn_{RUN_NO}.txt")
...
if face_cascade.empty():
    ...
    sys.exit(1)
print(f"[✓] Haar Cascade dimuat dari: {cascade_path}")
...
cv2.namedWindow(f"[{DETECTOR_NAME}-DNN] Gesture Classification", cv2.WINDOW_KEEPRATIO)
cv2.resizeWindow(f"[{DETECTOR_NAME}-DNN] Gesture Classification", 640, 480)
...
try:
    lines = [
        "=" * 80,
        "LAPORAN HASIL SISTEM HAAR CASCADE + DNN (GPU-Accelerated)",
        "=" * 80,
