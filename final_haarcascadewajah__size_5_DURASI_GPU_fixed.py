"""
Realtime Haar Cascade detector (OpenCV) - deteksi wajah (ya/tidak).
Kontrol:
...
CATATAN GPU:
File ini menggunakan OpenCV Haar Cascade murni (CPU).
...
def get_detector():
    ...
    if _haar_detector is None:
        ...
        if _haar_detector.empty():
            raise RuntimeError(f"Gagal memuat Haar Cascade dari {cascade_path}")
    return _haar_detector


def detect_face_haar(image):
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    return get_detector().detectMultiScale(gray, scaleFactor=1.2, minNeighbors=5, minSize=(24, 24), flags=cv2.CASCADE_SCALE_IMAGE)
...
def main():
    ...
    print("[INFO] Deteksi wajah real-time — Haar Cascade (OpenCV CPU)")
    ...
    while True:
        ...
        cv2.putText(frame, "Mode: CPU (Haar Cascade)", (10, FRAME_SIZE[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
    ...
    print("\n" + "=" * 80)
    print("CONFUSION MATRIX — Haar Cascade (OpenCV) | Mode: CPU")
    ...
    log_dir = "log_haarcascade_wajah_csv"
    log_base = "log_haar_wajah"
